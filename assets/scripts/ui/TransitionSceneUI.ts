...
export class TransitionSceneUI {
    ...
    show(): void {
        ...
        const scene = loader?.createComp(UI_COMPONENTS.transitionScene) as fgui.GComponent;
        ...
        if (scene) {
            ...
            const code = scene.getChild('txtCode') as fgui.GTextField;
            if (code) code.text = `💾 记录完成`;
